(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('underscore'), require('bbmn-core'), require('bbmn-utils')) :
	typeof define === 'function' && define.amd ? define(['exports', 'underscore', 'bbmn-core', 'bbmn-utils'], factory) :
	(factory((global.bbmn = global.bbmn || {}, global.bbmn.mixins = {}),global._,global.bbmn,global.bbmn.utils));
}(this, (function (exports,_$1,bbmnCore,bbmnUtils) { 'use strict';
...
var Mixin = function Mixin(Base) {
	return Base.extend({
		...
		getProperty: function getProperty(key, opts) {
			...
			var defaultGetArguments = bbmnUtils.betterResult(this, '_getPropertyArguments', { args: [this], default: [this] });
			var options = _$1.extend({
			...
			var value = bbmnUtils.betterResult(this, key, options);
			if (value == null && deep !== false) {
				value = bbmnUtils.betterResult(this.options, key, options);
			}
			return value;
		},
		...
		getOption: function getOption$$1(key, opts) {
			var defaultGetArguments = bbmnUtils.betterResult(this, '_getOptionArguments', { args: [this], default: [this] });
			var options = _$1.extend({
				deep: Mixin.defaults.deep,
				force: Mixin.defaults.force,
				args: defaultGetArguments
			...
			return bbmnUtils.getOption(this, key, options);
		},
		mergeOptions: function mergeOptions$$1() {
			...
			if (_$1.isString(keys)) keys = keys.split(/\s*,\s*/);

			_$1.each(keys, function (key) {
				var option = bbmnUtils.betterResult(values, key, _$1.extend({ force: false }, opts));
				if (option !== undefined) {
					_this[key] = option;
				}
			});
...
var index$5 = (function (Base) {
	return Base.extend({
		...
		_initializeChildrenable: function _initializeChildrenable(opts) {
			bbmnUtils.mergeOptions(opts, ['parent', 'root']);
		...
		initializeChildren: function initializeChildren() {
			...
			this._children = [];
			_$1(children).each(function (child) {
		...
		_initializeChild: function _initializeChild(arg) {
			...
			if (bbmnUtils.isKnownCtor(arg)) Child = arg;else if (_$1.isFunction(arg)) {

				var invoked = arg.call(this, this);
				return this._initializeChild(invoked);
			} else if (_$1.isObject(arg)) {
				Child = arg.Child;
				_$1.extend(options, _$1.omit(arg, 'Child'));
			}

			_$1.extend(options, bbmnUtils.getOption(this, 'childOptions'), { parent: this });
		...
		_getChildren: function _getChildren(items) {
			...
			if (exclude != null && !_$1.isArray(exclude)) opts.exclude = [exclude];

			if (!_$1.isFunction(filter)) delete opts.filter;

			var result = [];
			_$1(items).each(function (item, index) {

				if (!_this2._childFilter(item, index, opts)) return;

				if (_$1.isFunction(map)) item = map(item);
		...
		_childFilter: function _childFilter(item, index) {
			...
			if (_$1.isFunction(this.childFilter) && !this.childFilter(item, index, opts)) return;

			if (_$1.isArray(exclude) && exclude.indexOf(item) >= 0) return;

			if (_$1.isFunction(filter) && !filter.call(this, item, index, opts)) return;
		...
		getAllChildren: function getAllChildren() {
			...
			var options = _$1.omit(opts, 'includeSelf', 'map');

			var children = this.getChildren(options);
			var result = _$1(children).chain().map(function (child) {
			...
			if (_$1.isFunction(map)) {
				return _$1(result).chain().map(map).filter(function (f) {
					return !!f;
				}).value();
			} else {
				return result;
...
function getNestedResult(value, context, schema) {
	return value != null && _$1.isFunction(schema.nested) && schema.nested(value, context);
}

function getPropertySchema(model, key) {
	if (_$1.isFunction(model.getPropertySchema)) {
...
function getDisplayConfig(key, model, schema) {
	if (key == null) return {};
	return _$1.isFunction(model.getPropertyDisplayConfig) && model.getPropertyDisplayConfig(key) || schema && schema.display || {};
}
...
var index$6 = (function (Base) {
	...
	var Mixed = Base.extend({
		...
		get: function get(key) {
			...
			if (_$1.isFunction(opts.transform) && !opts.raw) {
				value = opts.transform.call(this, value, opts, this);
			}

			if (_$1.isFunction(prop.transform) && !opts.raw) {
			...
			if (opts.display === true) {
				...
				if (opts.alternative) {
					value = _$1.isFunction(display.alternative) && display.alternative.call(this, value, _$1.extend({}, opts, prop), this);
				} else if (_$1.isFunction(display.transform)) {
...
var index$8 = (function (Base) {
	return Base.extend({
		...
		_getCssClassString: function _getCssClassString() {
			...
			var classes = _$1(modifiers).reduce(function (hash, modifier) {
				if (modifier == null || modifier === '') {
					return hash;
				}
				var cls = void 0;
				if (_$1.isString(modifier)) {
					cls = modifier;
				} else if (_$1.isFunction(modifier)) {
					var builded = modifier.call(_this, _this.model, _this);
					cls = _$1.isString(builded) && builded || undefined;
				}
				cls && (hash[cls] = true);
				return hash;
			}, {});

			return _$1.keys(classes).join(' ');
		...
		_setupCssClassModifiers: function _setupCssClassModifiers() {
			...
			_$1(events).each(function (eventName) {
		...
		_getCssClassConfig: function _getCssClassConfig() {
			var cfg = _$1.extend({}, defaultCssConfig, this.getOption('cssClassConfig'));
			if (!cfg || _$1.size(cfg) == 0) return;
			return cfg;
		},
...
function buildRegionFunc(view, hash, context) {
	...
	} else if (_$1.isFunction(updateDom)) {
		updateDom.call(view, $el, view);
	}

	if (!region) {
		var definition = _$1.pick(hash, 'replaceElement', 'regionClass');
...
function normalizeNestedViewContextRegion(context) {
	...
	var regionName = _$1.isString(region) && region || context.regionName || context.name;

	if (_$1.isString(region) || region == null) {
		region = {};
	} else if (_$1.isFunction(region)) {
		region = region.call(this, context, this);
	}

	if (_$1.isObject(region)) {

		if (!region.name) region.name = regionName;
		var replaceElement = this.getOption('replaceNestedElement');
		context.region = _$1.extend({ replaceElement: replaceElement }, region);
		context.show = _$1.partial(buildRegionFunc, this, context.region, context);
	}
	return context;
}
...
var index$10 = (function (Base) {
	return Base.extend({
		...
		initializeNestedViews: function initializeNestedViews() {
			...
			_$1(nesteds).each(function (context, index) {

				var name = _$1.isString(index) ? index : context.name || _$1.uniqueId('nested');
		...
		buildNestedView: function buildNestedView(name) {
			...
			var context = _$1.isObject(name) ? name : _$1.isString(name) ? this.getNestedViewContext(name) : null;

			if (!context) return;
			var passedView = bbmnUtils.betterResult(context, 'view', { context: this, args: [this, this.model] });
			if (_$1.isFunction(context.template)) return context.template;else if (bbmnCore.isView(passedView)) {
				return passedView;
			} else {
		...
		getNestedViewRegion: function getNestedViewRegion(name) {
			var context = _$1.isObject(name) ? name : _$1.isString(name) ? this.getNestedViewContext(name) : null;
			return context && _$1.result(context, 'show');
		}
	});
});

exports.emptyFetchMixin = index;
exports.improvedIndexesMixin = index$1;
exports.nextCollectionViewMixin = index$2;
exports.customsMixin = index$3;
exports.getOptionMixin = Mixin;
exports.isFetchingMixin = index$4;
exports.childrenableMixin = index$5;
exports.smartGetMixin = index$6;
exports.cssClassModifiersMixin = index$8;
exports.nestedViewsMixin = index$10;
exports.destroyView = index$9;
exports.buildViewByKeyMixin = index$7;
