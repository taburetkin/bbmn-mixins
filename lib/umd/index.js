...
var index$8 = (function (Base) {
	return Base.extend({
		...
		_initOwnEntities: function _initOwnEntities() {
			...
			_.each(additional, function (context) {
				if (_.isFunction(context)) {
					context = context.call(_this);
				}
				if (!context.name) return;
				if (!bbmnUtils.isModelClass(context.class) && !bbmnUtils.isCollectionClass(context.class)) {
					return;
				}
				memo[context.name] = bbmnUtils.clone(context, { functions: true });
			});
		...
		registerNestedEntity: function registerNestedEntity(name, context) {
			...
			if (_.isFunction(context)) {
				var wrapper = function wrapper() {
					var contextInvoked = context.call(this);
