...
import { betterResult, buildViewByKey, getByPath, getOption, isCollection, isCollectionClass, isKnownCtor, isModel, isModelClass, mergeOptions } from 'bbmn-utils';
import $ from 'jquery';
...
function getUrlPattern() {
	...
	var path = this.urlPattern || '';
	return path.replace(/\{([^}]+)\}/g, function (match, group) {
		var value = getByPath(_this, group);
		return value;
	});
}
...
var ModelMixin = function ModelMixin(Base) {
	return Base.extend({
		...
		getBaseUrl: function getBaseUrl() {
			var base = _.result(this, 'urlRoot') || _.result(this.collection, 'url') || this.getUrlPattern();
			return base;
		},
		url: function url() {
...
var index$9 = (function (Base) {
	...
	var mixin = isModelClass(Base) ? ModelMixin(Base) : isCollectionClass(Base) ? CollectionMixin(Base) : throwError();
