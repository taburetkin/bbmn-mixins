	...
	return Base.extend({
		...
		mergeOptions: function mergeOptions$$1() {
			...
			_$1.each(keys, function (key) {
				var option = betterResult(values, key, _$1.extend({ force: false }, opts));
				if (option !== undefined) {
					_this[key] = option;
				}
			});
...
var index$5 = (function (Base) {
	return Base.extend({
		...
		_initializeChild: function _initializeChild(arg) {
			...
			if (isKnownCtor(arg)) Child = arg;else if (_$1.isFunction(arg)) {
			...
			_$1.extend(options, getOption(this, 'childOptions'), { parent: this });
...
var index$10 = (function (Base) {
	return Base.extend({
		...
		buildNestedView: function buildNestedView(name) {
			...
			if (_$1.isFunction(context.template)) return context.template;else if (isView(passedView)) {
				return passedView;
			} else {
...
export { index as emptyFetchMixin, index$1 as improvedIndexesMixin, index$2 as nextCollectionViewMixin, index$3 as customsMixin, Mixin as getOptionMixin, index$4 as isFetchingMixin, index$5 as childrenableMixin, index$6 as smartGetMixin, index$8 as cssClassModifiersMixin, index$10 as nestedViewsMixin, index$9 as destroyView, index$7 as buildViewByKeyMixin };
