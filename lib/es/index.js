...
import { betterResult, buildViewByKey, clone, getByPath, getOption, isCollection, isCollectionClass, isKnownCtor, isModel, isModelClass, mergeOptions } from 'bbmn-utils';
import $ from 'jquery';
...
var index$16 = (function (Base) {
	return Base.extend({
		...
		_initializeScrollHandler: function _initializeScrollHandler() {
			...
			this.on({
				'attach scroll:handling:on': function attachScrollHandlingOn() {
					return _this.delegateEvents({
						'scroll': _this._scrollHandler
					});
				},
				'detach scroll:handling:off': function detachScrollHandlingOff() {
  ...
  getScrollVariants(){
  	...
  	return result;
		...
		_scrollHandler: function _scrollHandler() {
			...
			var edges = this.checkElementEdge(scroll);
		...
		checkElementEdge: function checkElementEdge(position) {
			var edges = this.getElementEdges();
			var left = edges.left - position.left;
			var top = edges.top - position.top;
			if (left < 300) left = 0;
			if (top < 300) top = 0;
			return { left: left, top: top };
		},
		getElementEdges: function getElementEdges() {
			var left = this._shdata.dim.width;
			if (left == null) {
				left = this._shdata.dim.width = this.$el.get(0).scrollWidth - this.$el.width();
			}
			var top = this._shdata.dim.height;
			if (top == null) {
				top = this._shdata.dim.height = this.$el.get(0).scrollHeight - this.$el.height();
			}
			return { left: left, top: top };
		},
		_clearEdges: function _clearEdges() {
			delete this._shdata.dim.width;
			delete this._shdata.dim.height;
		},
		_tryTriggerEdges: function _tryTriggerEdges(edges) {
			var _this2 = this;

			var shouldTrigger = {};
			var elementEdges = this.getElementEdges();
			if (!edges.left) shouldTrigger.right = elementEdges.left;

			if (!edges.top) shouldTrigger.bottom = elementEdges.top;

			if (!_.size(shouldTrigger)) return;

			_(shouldTrigger).each(function (offset, edge) {
				_this2._triggerEdge(edge, offset);
			});
		},
		_triggerEdge: function _triggerEdge(edge, offset) {

			var storedOffset = this._shdata.off[edge];
...
export { index as emptyFetchMixin, index$1 as emptyViewMixin, index$2 as improvedIndexesMixin, index$3 as nextCollectionViewMixin, index$4 as customsMixin, index$8 as optionsMixin, index$5 as improvedFetchMixin, index$6 as childrenableMixin, index$7 as nestedEntitiesMixin, index$9 as urlPatternMixin, index$10 as smartGetMixin, index$11 as saveAsPromiseMixin, index$13 as cssClassModifiersMixin, index$15 as nestedViewsMixin, index$14 as destroyViewMixin, index$12 as buildViewByKeyMixin, index$16 as scrollHandlerMixin, index$17 as createAsPromiseMixin };
